...
def signature(timestamp, method, request_path, secret_key, body = None):
    if str(body) == '{}' or str(body) == 'None' or body == None:
        body = ''
    message = str(timestamp) + str.upper(method) + request_path + parse_params_to_str(body)
...
async def print_devices(base_url, request_path, body, header):
    async with httpx.AsyncClient(http2=True) as client:
        response = await client.get(base_url + request_path + parse_params_to_str(body), headers=header)
    print(response.http_version)


loop = asyncio.get_event_loop()
loop.run_until_complete(print_devices(base_url, request_path, body, header))
loop.close()
#response = httpx.get(base_url + request_path, data = body, headers=header)
# json
print(timestamp)
